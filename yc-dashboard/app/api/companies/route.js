...
export async function GET(request) {
  ...
  try {
    ...
    // 2. Batch Filter
    if (batch) {
      queryText += ` AND s.batch ILIKE $${counter}`; // Use ILIKE for partial match safety
      values.push(`%${batch}%`);
      counter++;
    }
    ...
    if (location) {
      queryText += ` AND s.location = $${counter}`;
    ...
    // Total Count (Approximate - ignoring filters for speed in this demo)
